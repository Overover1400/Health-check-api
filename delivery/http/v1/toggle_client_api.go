...
func ToggleClientApi(store store.MySqlStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		req := dto.ToggleClientApiRequest{
			ID: uint(clientApiIDToInt),
		}
		if err = clientApi.NewClientApi(store).ToggleClientApi(c.Request().Context(), req); err != nil {
			return echo.NewHTTPError(http.StatusInternalServerError, err.Error())
		}
